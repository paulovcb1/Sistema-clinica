...
HTML;
	for ($i = 0; $i < $total_reg; $i++) {
		...
		echo <<<HTML
			<tr> 
				<td><i class="fa fa-square {$classe_pago} mr-1"></i> {$descricao}</td> 
					<td style="text-align: left"  class="esc">R$ {$valorF}</td>	
		...
		<p>Confirmar Exclusão? <a href="#" onclick="excluirConta('{$id}', '{$descricao}')"><span class="text-danger">Sim</span></a></p>
					...
					<big><a class="{$ocultar_whats}" href="http://api.whatsapp.com/send?1=pt_BR&phone={$tel_pessoaF}" title="Whatsapp" target="_blank"><i class="fa-brands fa-whatsapp" style="color:green"></i></a></big>
				</td>  
			</tr> 
HTML;
	}
